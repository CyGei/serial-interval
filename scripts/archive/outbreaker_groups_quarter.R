...
outbreaker_groups_quarter <- function(df, group_column, individual_column, dates,w_params, f_params, outbreaker_config){
  start.time <- Sys.time()
  ...
  for (i in 1:length(unique_group_column)) {
    ...
    for (j in 1:10) {
      ...
      w_distribution_quarter <- vector()
      for (k in (1:100)) {
        val <- w_distribution$d(k)/4
        vec <- rep(val, 4)
        w_distribution_quarter<- c(w_distribution_quarter, vec)
      }
      
      f_distribution_quarter <- vector()
      for (k in (1:100)) {
        val <- f_distribution$d(k)/4
        vec <- rep(val, 4)
        f_distribution_quarter<- c(f_distribution_quarter, vec)
      }
      ...
      out_data <- outbreaker_data(
        ...
        w_dens = w_distribution_quarter,
        f_dens = f_distribution_quarter # incubation time distribution
      )
